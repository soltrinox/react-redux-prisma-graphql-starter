    ...
    return Array(amount)
      ...
      .map((k, i) => {
        ...
        const vars = Array(bezierSegmentation)
          .fill(1)
          .map((c, j) => {
            const height = endArea.y - startArea.y - startArea.height
            const y = (height / bezierSegmentation) * (j + 1)
            const x =
              Math.random() *
...
class MoneySnow extends React.Component {
  state = {
    show: true
  }
  onEnd = () => {
...
export {Snow, MoneySnow}

const MoneySnowStyles = styled.div`
  position: absolute;
  bottom: 0;
  top: 0;
  left: 0;
